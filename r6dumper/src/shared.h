...
struct StringReference {
    StringReference(uint64_t string_address = 0, int64_t offset_to_desired_address = 0, uint64_t reference_address = 0,
...
enum class Segment : uint8_t { Text = 0, Rdata, Data, All };
...
static uint64_t seg_end(Segment seg) {
    if (seg == Segment::All)
        return seg_end_arr[2]; // assuming data is last
...
static std::vector<uint64_t> sig_scan(const uint8_t* sig, size_t len, Segment seg = Segment::Text,
                                      const char* skips = "") {
    ...
    if (end_addr <= start_addr || end_addr < len)
        return ret;
...
static bool is_address_instruction(uint64_t address, bool relative = true) {
    return (memory[address] == 0x48 || memory[address] == 0x4C) &&
           (relative ? (memory[address + 1] == 0x8B || memory[address + 1] == 0x8D)
                     : (memory[address + 1] == 0xBB || memory[address + 1] == 0xBD || memory[address + 1] == 0xBE ||
                        memory[address + 1] == 0xBA || memory[address + 1] == 0xBF || memory[address + 1] == 0xB8));
}

// finds references to a variable in the .text segment
static std::vector<uint64_t> find_xrefs(uint64_t offset, uint8_t rex = 0, uint8_t instruction = 0, uint8_t reg = 0,
                                        bool relative = true) {
    ...
    threads.back() =
        new std::thread(search_loop, addr, seg_end(Segment::Text) - seg_start(Segment::Text) - size * (threads.size() - 1));
...
static std::vector<StringReference> find_string_refs_near(uint64_t offset) {
    std::vector<StringReference> refs;
    ...
    for (uint64_t i = offset - 0x100; i < offset + 0x100; i++) {
        ...
        refs.push_back(StringReference(address, i - offset, i));
...
static bool is_string_ref_unique(StringReference* str_ref) {
