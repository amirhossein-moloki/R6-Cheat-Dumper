...
void combat::norecoil(unsigned char recoil, float horizontal_recoil, float vertical_recoil, float spread) {
	...
	if (chain != 0)
		globals::memory.write<unsigned char>(chain + 0xE2D, recoil);
	...
	if (chain != 0) {
		globals::memory.write<unsigned char>(chain + 0x168, 0);
